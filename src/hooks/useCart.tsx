...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      if (!product && resStock.data.amount > 0) {
        ...
        localStorage.setItem(
          "@RocketShoes:cart",
          JSON.stringify([...cart, { ...resProducts.data, amount: 1 }])
        );
      } else if (product && product.amount <= resStock.data.amount) {
  ...
  const removeProduct = (productId: number) => {
    try {
      ...
      if (product) {
        const newListProduct = cart.filter(
